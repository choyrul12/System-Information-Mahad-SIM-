...
class NilaiBulananModel extends Model
{
    ...
    public function getNilaiByNisn($nisn, $mapel, $ta, $bulan)
    {
        $result = array();
        $mapel = $this->query("SELECT kd_mapel FROM tb_mapel WHERE kls_mapel = $_GET[kelas]  AND rapor_bulanan = 'Show' GROUP BY nm_mapel");

        foreach ($mapel->getResultArray() as $m) {
            $nilai = $this->select('ROUND(score,0) AS score')->getWhere(['nisn' => $nisn, 'kd_mapel' => $m['kd_mapel'], 'kd_ta' => $ta, 'bulan' => $bulan]);
            if ($nilai->getNumRows() > 0) {
                $score = $nilai->getRow('score');
            } else {
                $score = 0;
