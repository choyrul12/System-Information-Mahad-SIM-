...
class MapelModel extends Model
{
    ...
    public function getListMapel($kls, $rapor)
    {
        $unit = session()->get('unit');
        $result = $this->groupBy('nm_mapel')->getWhere(['unit' => $unit, 'kls_mapel' => $kls, $rapor => 'Show']);
        return $result;
    }

    public function getDataMapelByKls($id)
    {
        $result = $this->orderBy('urutan_mapel_bulanan ASC')->getWhere(['kls_mapel' => $id]);
        return $result;
    }

    public function getDataMapelByKelompok($id, $kelompok)
    {
        $result = $this->query("SELECT * FROM `tb_mapel` WHERE `kelompok_mapel` = '$kelompok' AND `kls_mapel` = '$id' AND rapor_semester = 'Show' order by `urutan_mapel` asc");
        return $result;
    }
