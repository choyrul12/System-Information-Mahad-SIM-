...
class NilaiMapelModel extends Model
{
    ...
    {
        $result = $this->getWhere(['kd_mapel'=>$mapel,'nisn' => $nisn, 'kd_ta' => $ta])->getRow();
        return $result;
    }
    ...
    {
        $result = array();
        $mapel = $this->query("SELECT kd_mapel FROM tb_mapel WHERE kls_mapel = $_GET[kelas]");
        
        foreach($mapel->getResultArray() AS $m){   
            $nilai = $this->select('COALESCE(na,0) AS p, COALESCE(keterampilan,0) AS k')->getWhere(['nisn' => $nisn,'kd_mapel' => $m['kd_mapel'],'kd_ta' => $ta]);
            if($nilai->getNumRows() > 0){
                $p = $nilai->getRow('p');
                $k = $nilai->getRow('k');
            }else{
