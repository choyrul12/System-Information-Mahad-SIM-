...
class Administrator extends BaseController
{
    ...
    public function listMenu()
    {
        ...
        $dataLedger = array("title"=>"Data Ledger", "link"=>"/administrator/data-ledger", "icon"=>"nav-icon fas fa-table", "id" => "data-ledger");
        ...
        switch($section){
            ...
            case 'ACADEMIC':
                ...
                if($level == '2'){
                    ...
                    $data["menu_tree"] = array($dataNilaiMapel,$dataNilaiAfektif,$dataNilaiEskul,$dataAbsensi,$standardKkm,$standardPengetahuan,$standardKeterampilan,$standardAfektif,$standardEskul);
                }elseif($level == '3'){
                    ...
                    $data["menu_tree"] = array($dataNilaiMapel,$dataNilaiAfektif,$dataNilaiEskul);
                }
                return $data; break;
        }
    }
    ...
    public function showDataLedger()
    {
        ...
        $data['list_mapel'] = $this->mapelModel->getListMapel($kelas); 
        $data['list_pesdik'] = $this->pesdikModel->getListPesdik($rombel);
        $data['kkm'] = $this->gradeModel->getKkm($unit);
        $section = $this->section;
        if ($this->isLogin() && ($section == "IT"||$section == "ACADEMIC")) {
            return view('admin/pages/data_ledger',$data);
        }else{
            return redirect()->to('/');
        }
    }
